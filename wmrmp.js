        ...
        if (processedProfessors.indexOf(professorName) === -1) { //the professor has not been processed yet
            ...
            if (exceptions[professorName]) {
                ...
                if (courseId.indexOf('LAW') == -1) { //The Law school is listed separately in RMP
                    ...
                    rmpsearch = 'http://www.ratemyprofessors.com/search.jsp?queryBy=teacherName&schoolName=college+of+william+and+mary&queryoption=HEADER&query=PROFESSORNAME&facetSearch=true';
                    rmpsearch = rmpsearch.replace('PROFESSORNAME', URLprofessorName);
                } else {
                    //Create Rate My Professor search URL using the professor name in the correct form.
                    rmpsearch = 'http://www.ratemyprofessors.com/search.jsp?queryBy=teacherName&schoolName=william+&+mary+school+of+law&queryoption=HEADER&query=PROFESSORNAME&facetSearch=true';
                    rmpsearch = rmpsearch.replace('PROFESSORNAME', URLprofessorName);
                }
